...
pub fn system(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    if extractor.field_count.is_some() {
        let context_struct_snake = extractor
            .context_struct_name
            .as_ref()
            .map(|name| name.to_lowercase())
            .unwrap_or_else(|| "components".to_string());
        let client_accounts_mod: proc_macro2::TokenStream =
            format!("__client_accounts_{}", context_struct_snake)
                .parse()
                .unwrap();
        let cpi_client_accounts_mod: proc_macro2::TokenStream =
            format!("__cpi_client_accounts_{}", context_struct_snake)
                .parse()
                .unwrap();
        ...
        let expanded = quote! {
            #[program]
            #ast
            pub(crate) use #module_ident::#client_accounts_mod;
            pub(crate) use #module_ident::#cpi_client_accounts_mod;
        };
    ...
    fn add_variadic_execute_function(content: &mut Vec<syn::Item>) {
        content.push(syn::parse2(quote! {
            pub fn bolt_execute<'info>(ctx: Context<'_, '_, 'info, 'info, VariadicBoltComponents<'info>>, args: Vec<u8>) -> Result<Vec<Vec<u8>>> {
                ...
                let bumps = ComponentsBumps::default();
