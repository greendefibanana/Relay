    ...
    (
        quote! {
            ...
            pub fn update(ctx: Context<Update>, data: Vec<u8>) -> Result<()> {
                let bolt_component = {
                    let bolt_component_data = ctx.accounts.bolt_component.try_borrow_data()?;
                    let mut bolt_component_bytes: &[u8] = &bolt_component_data;
                    <#component_type as anchor_lang::AccountDeserialize>::try_deserialize(&mut bolt_component_bytes)?
                };

                require!(bolt_component.bolt_metadata.authority == World::id() || (bolt_component.bolt_metadata.authority == *ctx.accounts.authority.key && ctx.accounts.authority.is_signer), BoltError::InvalidAuthority);
                ...
                let updated_component = <#component_type>::try_from_slice(&data)?;
                let mut bolt_component_data = ctx.accounts.bolt_component.try_borrow_mut_data()?;
                let mut writer = std::io::Cursor::new(&mut bolt_component_data[..]);
                updated_component.try_serialize(&mut writer)?;
                Ok(())
            }
        },
        quote! {
            ...
            pub fn update_with_session(ctx: Context<UpdateWithSession>, data: Vec<u8>) -> Result<()> {
                ...
                if bolt_component.bolt_metadata.authority == World::id() {
                    ...
                    let validity_ctx = bolt_lang::session_keys::ValidityChecker {
                        ...
                        authority: bolt_component.bolt_metadata.authority.clone(),
                    ...
                    require_eq!(bolt_component.bolt_metadata.authority, ctx.accounts.session_token.authority, bolt_lang::session_keys::SessionError::InvalidToken);
                }
        ...
        quote! {
            ...
            pub struct Update<'info> {
                #[account(mut)]
                /// CHECK: Delegated components are owned by the delegation program.
